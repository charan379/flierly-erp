 ...
 */
function applyFindSort(sort: Record<string, SortOrder>): FindOptionsOrder<ObjectLiteral> {
    const order: FindOptionsOrder<ObjectLiteral> = {};

    Object.entries(sort).forEach(([key, value]) => {
        order[key] = (value === 'ascend' || value === 'asc' || value === 'ascending' || value === 1)
            ? 'ASC'
            : 'DESC'; // Default to DESC if not ascending
    });

    return order;
}

export default applyFindSort;
