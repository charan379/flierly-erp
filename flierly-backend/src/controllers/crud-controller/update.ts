...
const update = async (entity: EntityTarget<ObjectLiteral>, req: Request, res: Response): Promise<Response> => {
    ...
    const result = await AppDataSource.transaction(async (entityManager) => {
        ...
        if (!existingEntity) throw new FlierlyException(`${repo.metadata.name} not found with ID: ${id}`, HttpCodes.BAD_REQUEST, `${repo.metadata.name} not found with ID: ${id}, Invalid Update`);
