...
interface updateAssociatedRecordsRequestBody {
...
const updateAssociatedRecordsRequestBodySchema: Joi.ObjectSchema<updateAssociatedRecordsRequestBody> = Joi.object({
...
const updateAssociatedRecords = async (
    owningEntity: EntityTarget<ObjectLiteral>,
    req: Request,
    res: Response
): Promise<Response> => {
    // Validate request body
    const { owningEntityId, inverseField, newArray, addOne, removeOne, addMultiple, removeMultiple }: updateAssociatedRecordsRequestBody = await JoiSchemaValidator(
        updateAssociatedRecordsRequestBodySchema,
        req.body,
        { abortEarly: false },
...
export default updateAssociatedRecords;