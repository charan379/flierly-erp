...
const authenticate = async (req: Request, res: Response) => {
  // validate new user
  const credentials: { username: string; password: string } = await JoiSchemaValidator(
    credentialsSchema,
    req.body,
  ...
  if (userPassword === null)
    throw new FlierlyException(
      ...
      'authenticate-user-controller-password-not-generated',
    );
  // validate hash
  ...
  if (!isPasswordValid) throw new FlierlyException('Invalid password', HttpCodes.BAD_REQUEST, 'Password does not match', 'authenticate-user-controller-invalid-password');
