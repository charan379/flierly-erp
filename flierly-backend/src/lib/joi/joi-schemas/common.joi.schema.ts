...
export const idSchema = Joi.number().required().messages({
  'number.base': 'Please provide a valid id: {#value} is not a valid number.',
  'any.required': 'A id is required.',
});

export const idArraySchema = Joi.array().items(Joi.number().required()).required().messages({
...
export const nameMi5Ma50Schema: Joi.Schema<string> = Joi.string().min(5).max(50).example('Name').messages({
  'string.min': 'Name must be at least 5 characters long.',
  'string.max': 'Name must not exceed 50 characters.',
});

export const emailSchema: Joi.Schema<string> = Joi.string().email().example('someone@gmail.com').messages({
  'string.email': 'Invalid email address',
});

export const phoneSchema: Joi.Schema<string> = Joi.string().min(10).max(13).example('9876543210').messages({
