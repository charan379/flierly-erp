                    ...
                    return (
                        <Flex gap={0} wrap={"nowrap"} align="end">
                            <Tooltip title={translate("search")}>
                                <Button key={"submit"} onClick={props.submit} style={{ borderTopRightRadius: "0px", borderBottomRightRadius: "0px", border: "none" }} icon={<SearchOutlined />} type="primary" />
                            </Tooltip>
                            <Tooltip title={translate("clear_filter")}>
                                <Button key={"reset"} onClick={props.reset} style={{ borderTopLeftRadius: "0px", borderBottomLeftRadius: "0px", border: "none" }} icon={<CloseOutlined />} type="primary" danger />
                            </Tooltip>
                        </Flex >
                    )
                },
                ...
                <ProFormDependency name={['queryField']}>
                    {({ queryField }) => {
                        const selectedField = filterConfig.find(
                            (config) => config.name === queryField
                        );
                        if (selectedField) {
                            return (
                                <div className="query-form-item" style={{ width: "180px", maxWidth: "180px" }}>
                                    <FormField
                                        config={{ ...selectedField.formField, formInfo: { gridForm: false }, name: "query" }}
                                        showLabel={false}
                                    />
                                </div>
                            );
