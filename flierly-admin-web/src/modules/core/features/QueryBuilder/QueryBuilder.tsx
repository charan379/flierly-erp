...
import queryTransformers, { TransformerKey } from '@/modules/core/utils/query-transformers'
...
export type QueryBuilderFieldConfig<T = Record<string, any>> = {
  field: { label: string; namePath: keyof T }
  conditions: {
    condition: { label: string; namePath: TransformerKey }
    queryFieldConfig: QueryFieldConfig<any>
  }[]
}
...
interface QueryBuilderProps {
  config: QueryBuilderFieldConfig<any>[]
...
const QueryBuilder = forwardRef<QueryBuilderRef, QueryBuilderProps>(({ config, initialConditions = [] }, ref) => {
        ...
        cond.id === id
          ? {
            ...
            field: selectedField?.field ? {
              ...
              namePath: String(selectedField.field.namePath)
            } : undefined,
            condition: undefined,
      ...
      prev.map((cond) => {
        if (cond.id === id) {
          ...
          return {
            ...
            queryFieldConfig: selectedCondition?.queryFieldConfig,
  ...
  const getAvailableFields = (): QueryBuilderFieldConfig[] => {
    const usedFields = conditions.map((cond) => cond.field?.namePath)
    return config.filter((qbc) => !usedFields.includes(qbc.field.namePath as string))
  }
                ...
                {cond.queryFieldConfig && (
                  <Form>
                    <QueryField
                      ...
                      config={{
                        ...cond.queryFieldConfig, // Pass dynamic form configuration
